...
echo "=========================================="
echo "Image-CoT Training with DeepSpeed"
echo "=========================================="
...
CONFIG=${CONFIG:-configs/stage1_config_qwen3vl_4b.yaml}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--num_gpus N] [--deepspeed_config PATH] [--config PATH] [--dataset NAME] [--batch_size N] [--num_epochs N] [--lr LR] [--save_interval N] [--resume_from_checkpoint PATH] [--master_port PORT]"
            exit 1
            ;;
    esac
done
...
if [ -n "$SAVE_INTERVAL" ]; then
    DEEPSPEED_ARGS+=(--save_interval "$SAVE_INTERVAL")
fi
